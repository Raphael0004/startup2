...
# Standard Scaling the colunmns indicidually
...
st.markdown("<h4 style = 'margin: -30px; color: #F11A7B; text-align: center; font-family: cursive '>Built By Raphael Madugu Teams</h4>", unsafe_allow_html = True)
...
# st. subheader('Tranformed input variables')
# st.dataframe(inputs)

Prediction_button = st.button('Predict Profitability')
if Prediction_button:
  predicted = lin_reg.predict(inputs)
  st.success(f'The Profit Predicted for your company is {predicted[0].round()}')

# for i in inputs.columns:
#     scaler.transform(inputs[[i]])
